...
compute_weighted_xis            = ft.partial(t3p.compute_xis, xnp=jnp)
compute_weighted_mus            = ft.partial(t3p.compute_mus, xnp=jnp)
compute_weighted_nus            = ft.partial(t3p.compute_nus, xnp=jnp)
compute_weighted_etas           = ft.partial(t3p.compute_etas, xnp=jnp)
assemble_probes                 = ft.partial(t3p.assemble_zs, xnp=jnp)
probe_tangent                   = ft.partial(t3p.probe_tangent, use_jax=True)
compute_weighted_dxis           = ft.partial(t3p.compute_dxis, xnp=jnp)
compute_weighted_sigmas         = ft.partial(t3p.compute_sigmas, xnp=jnp)
compute_weighted_taus           = ft.partial(t3p.compute_taus, xnp=jnp)
compute_weighted_detas          = ft.partial(t3p.compute_detas, xnp=jnp)
assemble_tangent_probes         = ft.partial(t3p.assemble_tangent_zs, xnp=jnp)
compute_weighted_deta_tildes    = ft.partial(t3p.compute_deta_tildes, xnp=jnp)
compute_weighted_tau_tildes     = ft.partial(t3p.compute_tau_tildes, xnp=jnp)
compute_weighted_sigma_tildes   = ft.partial(t3p.compute_sigma_tildes, xnp=jnp)
compute_weighted_dxi_tildes     = ft.partial(t3p.compute_dxi_tildes, xnp=jnp)
