...
__all__ = [
    # The grouped-einsum interpreter (the general entry point; the named contractions below are
    # each equivalent to one contract(...) call and slated for retirement in its favor).
    'contract',
    'Wa_Caib_Wi_to_WCb',
...
# --------------------------------------------------------------------------------------------------
# One general entry point replacing the enumerated named contractions. The subscripts use the
# standard einsum format with one extension: an UPPERCASE letter is a GROUP of zero or more axes
# (W probe stack, K tangent stack, C frame stack, ...); lowercase letters are single axes as usual.
# The interpreter solves each group's axis count from the operand ndims (a small exact linear
# system; see `_solve_group_lengths`), expands every group into fresh single-axis letters, and runs
# ONE ordinary einsum on the operands EXACTLY AS GIVEN.
#
# No reshape ever happens -- this is what retires the SHARDING block's residue above: there is no
# flatten, so EVERY axis of EVERY group is shardable (not just the leading one), and no fusion of
# two groups is even expressible. The numpy pairwise path is computed on the GROUPED string (one
# symbol per group -- size-independent and identical to the paths the named contractions use) and
# applied to the expanded einsum; jax gets the single expanded einsum, as always.
...
def contract(
    ...
    This is the general form of this module's named contractions: ``WCa_Caib_WCi_to_WCb(...)`` is
    ``contract('WCa,Caib,WCi->WCb', ...)``. Block-letter conventions (``W`` probe stack, ``K``
    tangent stack, ``C`` frame stack, frame-inner order) are in ``docs/batching_and_stacking.md``.
    Group axes follow einsum's usual extent rules: mismatched sizes for the same group axis raise,
    except that a size-1 axis broadcasts (standard einsum semantics on both backends).
