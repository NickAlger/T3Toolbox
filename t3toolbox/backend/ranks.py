...
def compute_continuation_ranks(
    ...
    ``r_0, r_d`` are never grown.

    .. warning::
        **Do not pass uniform (``ut3svd``) singular values here.** This is a ragged, host-side function
        and it infers the current ranks from ``len(s[i])``. Uniform spectra are padded to the supercore's
        *nominal* rank, so every ``len`` is the padded width rather than the true rank, and the padding
        zeros make ``kappa_i`` infinite -- the call type-checks and returns silently wrong ranks (in a
        measured case ``((2,2,2),(1,2,2,1))`` where the truth was ``((3,3,3),(1,3,3,1))``, i.e. a
        spurious "stop continuing"). Rank continuation is deliberately a ragged-layer activity: run the
        bookkeeping on the ragged point and drop into the uniform layer only for the fit itself
        (``docs/rank_continuation.md``, "On the uniform layer").

    Proposed ranks are then de-degenerated by
