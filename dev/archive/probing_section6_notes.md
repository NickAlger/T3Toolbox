...
| Paper | code | meaning |
|---|---|---|
| `ξᵢ = Uᵢᵀwᵢ` (contract up) | `compute_xi` | probe vec through Tucker basis |
| `μᵢᵀ = μᵢ₋₁ᵀ Gᵢ(ξᵢ)` (left sweep) | `compute_mu` | left partial product |
| `νᵢ = Gᵢ(ξᵢ) νᵢ₊₁` (right sweep) | `compute_nu` | right partial product |
| `ηᵢ = μᵢ₋₁ᵀ Gᵢ νᵢ` (central) | `compute_eta` | leaves Tucker-middle free |
| `zᵢ = Uᵢ ηᵢ` (contract down) | `assemble_z` | back to ambient index |

`t3_probe` = Algorithm 5 (already wrapped by `TuckerTensorTrain.probe`).
...
- **Algorithm 6** = Algorithm 5 on the *base point* with gauge-appropriate cores **`P` (left),
  ...
  In code: `compute_xi/mu/nu/eta` on `(U,P,Q,O)`.
...
`tv_probe` = Algorithm 6 + Algorithm 7 = `𝒥⁽ˢ⁾`.
...
= **`tv_probe_transpose`** (`compute_deta_tilde/tau_tilde/sigma_tilde/dxi_tilde` +
...
2. **Frame-core ordering**: `tv_probe`/`tv_probe_transpose` take
...
3. **Factorization not assembled**: `tv_probe` is `𝒥⁽ˢ⁾`, `orthogonal_gauge_projection` is `Π`,
...
4. **Frame edge-var caching**: `ξ̂,μ̂,ν̂,η̂` depend on `(p,s)` not on the tangent vector → can be
   precomputed once per (frame, sample) and reused (compute↔memory trade-off). `tv_probe`
   currently recomputes them every call.
5. **Stacking/vectorization**: both directions now batch over all three blocks — `W` probes, `K`
   tangent stack, `C` frame stack. Forward (`tv_probe`): the `K`-stacked case via 3-group
   contractions in `compute_sigma`/`compute_deta`, `assemble_tangent_z` (slice 5c; output `W + K + C`).
   Transpose (`tv_probe_transpose`): accepts `K`-stacked residuals `W + K + C` and carries `K`
...
- Wire `tv_probe` / `tv_probe_transpose` into `T3Tangent` (`.probe()` and the transpose),
  reconciling the frame order, like the manifold port.
- Compose the Riemannian `J` / `Jᵀ` (`𝒥∘Π`, `Π∘𝒥ᵀ`) as callables on `T3Tangent`.
- Tests against the dense reference (`dense_probe`) + the `Jᵀ` adjoint identity
  `⟨z̃, 𝒥(δV)⟩ = ⟨𝒥ᵀ(z̃), δV⟩`.
