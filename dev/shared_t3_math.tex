...
\begin{remark}[Truncation error identity]
When the input is exactly shared and step (4) truncates group $g$ from
$n_g$ to $n_g'$, the error contributed by that step is
$\bigl(\sum_{j > n_g'} s_{g,j}^2\bigr)^{1/2}$: by
Lemma~\ref{lem:skeleton} the discarded tail of the concatenation is exactly
the Hilbert--Schmidt error of simultaneously projecting all group modes onto
the retained basis.
\end{remark}
...
formula in the other geometry projects onto the wrong subspace; the two agree
only when the group Grams are all equal (e.g.\ at an exactly symmetric
point, Corollary~\ref{cor:sym}).
\end{remark}
